...
static MarkPdata(mainbase, segbase) // debug directory
{
	...
	addr = segbase+((count-1)*8);
	vaddr = Dword(addr);
	Message(form("Labeling Functions by Debug table\ntotal: %08x base %08x addr: %08x vaddr:%08x\n", count, segbase, addr, vaddr));
	while(addr >= segbase)
	{
		vaddr = Dword(addr);
		excp = Dword(addr+4);
...
static MarkXedata(mainbase, segbase, codestart, codeend) // Xbox export directory big endian
{
	...
	for(i=count; i>0; i--)
	{
		...
		if(stat != 0)
		{
			...
			{
				if((strstr(name, "KeTimeStampBundle") == 0) && (GetStrucIdByName("_KTIME_STAMP_BUNDLE") != -1))
				{
					MakeStructEx(stat, -1, "_KTIME_STAMP_BUNDLE");

				}
				else if((strstr(name, "XboxHardwareInfo") == 0) && (GetStrucIdByName("_XBOX_HARDWARE_INFO") != -1))
				{
					MakeStructEx(stat, -1, "_XBOX_HARDWARE_INFO");
				}
				else if((strstr(name, "XboxKrnlVersion") == 0) && (GetStrucIdByName("_XBOX_KRNL_VERSION") != -1))
				{
					MakeStructEx(stat, -1, "_XBOX_KRNL_VERSION");
				}
				else if((strstr(name, "XboxKrnlBaseVersion") == 0) && (GetStrucIdByName("_XBOX_KRNL_VERSION") != -1))
				{
					MakeStructEx(stat, -1, "_XBOX_KRNL_VERSION");
				}
				else
...
// {"8e000000", "00010000"},//Virtual: 0x8e000000 Physical: 0x01ef0000 Protect: 0x0002 (System 64K)
// {"8e010000", "00010000"},//Virtual: 0x8e010000 Physical: 0x01f00000 Protect: 0x0002 (System 64K)
// {"8e030000", "00010000"},//Virtual: 0x8e030000 Physical: 0x01f10000 Protect: 0x0002 (System 64K)
// {"8e050000", "00010000"},//Virtual: 0x8e050000 Physical: 0x01f20000 Protect: 0x0002 (System 64K)
// {"90001000", "00001000"},//Virtual: 0x90001000 Physical: 0x01f31000 Protect: 0x0002 (Image 4K)
// {"90002000", "00001000"},//Virtual: 0x90002000 Physical: 0x01f32000 Protect: 0x0004 (Image 4K)
...
{
	if(loadSectionFile(0x8e000000, "8e000000.bin") == 1) // CRL data
		setupCrlCacheSection(0x8e000000);

	loadSectionFile(0x8e010000, "8e010000.bin");

	if(loadSectionFile(0x8e030000, "8e030000.bin") == 1)
		setupDiskCacheSection(0x8e030000);

	if(loadSectionFile(0x8e050000, "8e050000.bin") == 1)
		setupXexCacheSection(0x8e050000);
...
static MarkSyscalls(cbase)
{
	...
	while(addr != BADADDR)
	{
		...
		if(addr != BADADDR)
		{
			raddr = addr-4;
			tshort = Word(addr-2);
			if(MakeNameEx(raddr, form("%s_%x",myGetSyscallName(tshort), tshort), SN_NOCHECK|SN_NOWARN) != 1)
			{
				i = 0;
				//while(MakeNameEx(raddr, form("%s_%d_%x",GetSyscallName(tshort), i, tshort), SN_NOCHECK|SN_NOWARN) != 1)
				//	i = i+1;
				if(MakeNameEx(raddr, form("%s",GetSyscallName(tshort)), SN_NOCHECK|SN_NOWARN) != 1)
				{
					while(MakeNameEx(raddr, form("%s_%d",GetSyscallName(tshort), i), SN_NOCHECK|SN_NOWARN) != 1)
						i = i+1;
				}
			}
			Message(form("syscall marked: %s syscall %x 0x%08x\n",GetSyscallName(tshort), tshort, raddr));
		}
	}
}
...
static main()
{
	...
	SetCharPrm(INF_MODEL, 0x53); // sets calling conv cdecl, memory model "code near,data near" - used idc cmd line GetCharPrm(INF_MODEL) after setting manually to get this value
    ...
    {
		Til2Idb(-1, "_STRING");
		Til2Idb(-1, "_DRIVER_OBJECT");
		Til2Idb(-1, "_KTIME_STAMP_BUNDLE"); // KeTimeStampBundle
		Til2Idb(-1, "_XBOX_HARDWARE_INFO"); // XboxHardwareInfo
		Til2Idb(-1, "_XBOX_KRNL_VERSION"); // XboxKrnlVersion
    }
	else
